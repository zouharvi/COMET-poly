...
if args.mode == "train":
    ...
    recipe = [(src_to_tgts_dev, f"data/csv/dev_multitask.csv"), (src_to_tgts_train, f"data/csv/train_multitask.csv")]
elif args.mode == "test":
    recipe = [(src_to_tgts, f"data/csv/test_multitask.csv")]
    ...
    for (src, langs), tgts in tqdm.tqdm(src_to_tgts):
        for (mt1, score1), (mt2, score2) in itertools.combinations(tgts, 2):
            ...
            data_out.append({
                ...
                "score_da1": score1,
                "score_da2": score2,
                "score_pw": (score1 > score2) * 1,
            })


    with open(f_data_out, "w") as f:
        writer = csv.DictWriter(f, fieldnames=["src", "mt1", "mt2", "score_da1", "score_da2", "score_pw"])
        writer.writeheader()
...
"""
python3 experiments/03a-generate_multitask_data.py train -t 0
python3 experiments/03a-generate_multitask_data.py test -t 0
"""
