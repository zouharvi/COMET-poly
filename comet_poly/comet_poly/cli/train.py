...
e.g:
```
    comet-poly-train --cfg configs/models/regression_metric.yaml --seed_everything 12
```

For more details run the following command:
```
    comet-poly-train --help
```
"""
import json
import logging
...
def read_arguments() -> ArgumentParser:
    ...
    parser.add_subclass_arguments(PolyCandMetric, "polycand_metric")
    parser.add_subclass_arguments(PolyICMetric, "polyic_metric")
...
def initialize_model(configs):
    ...
    elif configs.polycand_metric is not None:
        print(
            json.dumps(
                configs.polycand_metric.init_args, indent=4, default=lambda x: x.__dict__
            )
        )
        if configs.load_from_checkpoint is not None:
            logger.info(f"Loading weights from {configs.load_from_checkpoint}.")
            model = PolyCandMetric.load_from_checkpoint(
                checkpoint_path=configs.load_from_checkpoint,
                strict=configs.strict_load,
                **namespace_to_dict(configs.polycand_metric.init_args),
            )
        else:
            model = PolyCandMetric(**namespace_to_dict(configs.polycand_metric.init_args))
    elif configs.polyic_metric is not None:
        print(
            json.dumps(
                configs.polyic_metric.init_args, indent=4, default=lambda x: x.__dict__
            )
        )
        if configs.load_from_checkpoint is not None:
            logger.info(f"Loading weights from {configs.load_from_checkpoint}.")
            model = PolyICMetric.load_from_checkpoint(
                checkpoint_path=configs.load_from_checkpoint,
                strict=configs.strict_load,
                **namespace_to_dict(configs.polyic_metric.init_args),
            )
        else:
            model = PolyICMetric(**namespace_to_dict(configs.polyic_metric.init_args))
